...
for (j in 1:length(file.names)){
  j
gg<-NULL
  g.dev[[j]] <- ggplot(df.table.dev[[j]],aes (x = bug_type,y = value)) + ggtitle("# of Developer who changed files")+ geom_boxplot(outlier.colour = "black",outlier.shape = 4,outlier.size = 4)
  g.chu[[j]] <- ggplot(df.table.chu[[j]],aes (x = bug_type,y = value)) + ggtitle("Code churn")+ geom_boxplot(outlier.colour = "black",outlier.shape = 4,outlier.size = 4)
  g.time1[[j]] <- ggplot(df.table.time1[[j]],aes (x = bug_type,y = value)) + ggtitle("Open to Last Commit (hours)")+ geom_boxplot(outlier.colour = "black",outlier.shape = 4,outlier.size = 4)                                      
  g.time2[[j]] <- ggplot(df.table.time2[[j]],aes (x = bug_type,y = value)) + ggtitle("Open to Resolved (hours)")+ geom_boxplot(outlier.colour = "black",outlier.shape = 4,outlier.size = 4)
  pdf(paste(paste(path,project[j],sep="/"),"_boxplot_hours_normal.pdf",sep=""),width=12,height=10)
